...
​용어 사용: 경제/정치/시사 분야의 **통용 약어(YoY, QoQ, BP, YTD 등)**와 **전문 용어(매파/비둘기파, 숏커버링, 펀더멘털 등)**를 그대로 사용하여 전문가스러운 톤을 유지하세요.
긴 서술보다는 함축적인 한자어(예: 상승하다→상회, 지켜보다→관망, 걱정하다→우려)를 적극 사용하여 문장 길이를 압축하세요.
상승/하락/보합 등의 방향성은 텍스트 대신 **특수기호(↑, ↓, -)**를 적극 활용하여 직관성을 높이세요.
문장은 건조한(Dry) 톤을 유지하되, 어닝 서프라이즈, 밸류에이션, 모멘텀 등 전문 용어를 사용하여 정보 밀도를 높이세요.
